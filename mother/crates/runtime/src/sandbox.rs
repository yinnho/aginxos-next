...
//! # Host ABI
//!
//! The host provides (in the `"carrier"` import module):
...
impl WasmSandbox {
    ...
    /// Register host function imports in the linker ("carrier" module).
            ...
            .func_wrap(
                ...
                |mut caller: Caller<'_, GuestState>,
                 ...
                 -> wasmtime::Result<i64> {
                    ...
                    if end > data.len() {
                        wasmtime::bail!("host_call: request out of bounds");
                    }
                    ...
                    if dest_end > mem_data.len() {
                        wasmtime::bail!("host_call: response exceeds memory bounds");
                    }
            ...
            .func_wrap(
                ...
                |mut caller: Caller<'_, GuestState>,
                 ...
                 -> wasmtime::Result<()> {
                    ...
                    if end > data.len() {
                        wasmtime::bail!("host_log: pointer out of bounds");
                    }
...
mod tests {
    ...
    const HOST_CALL_PROXY_WAT: &str = r#"
        (module
            (import "carrier" "host_call" (func $host_call (param i32 i32) (result i64)))

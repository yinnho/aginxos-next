...
use tracing::debug;
use carrier_types::capability::{capability_matches, Capability};
...
/// SSRF protection using the shared carrier_types::ssrf module.
fn is_ssrf_target(url: &str) -> Result<(), serde_json::Value> {
    carrier_types::ssrf::check_ssrf(url).map_err(|e| json!({"error": e.to_string()}))
}

/// Extract host from URL for capability checking.
fn extract_host_from_url(url: &str) -> String {
    carrier_types::ssrf::extract_host(url)
}
...
mod tests {
    ...
    fn test_is_private_ip() {
        use std::net::IpAddr;
        assert!(carrier_types::ssrf::is_private_ip(
    ...
    fn test_extract_host_from_url() {
        assert_eq!(
            carrier_types::ssrf::extract_host("https://api.openai.com/v1/chat"),
            "api.openai.com:443"
        );
        assert_eq!(
            carrier_types::ssrf::extract_host("http://localhost:8080/api"),
            "localhost:8080"
        );
        assert_eq!(
            carrier_types::ssrf::extract_host("http://example.com"),
            "example.com:80"
        );
    }
}

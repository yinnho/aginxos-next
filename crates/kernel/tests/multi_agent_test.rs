...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p carrier-kernel --test multi_agent_test -- --nocapture

use carrier_kernel::CarrierKernel;
use carrier_types::agent::AgentManifest;
use carrier_types::config::{DefaultModelConfig, KernelConfig};

fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("carrier-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = CarrierKernel::boot_with_config(test_config()).expect("Kernel should boot");

//! 公共文件浏览 URL —— runtime `file_view.rs` 的同构移植（M32 D3 批2）。
//!
//! file_write / file_convert 写到 sender 数据目录（output/…）后要附
//! view_url，让模型直接贴可点击链接。URL 形状与 runtime 完全一致：
//! `{external_url}/api/files/view/{agent_name}/output/foo.md?sender_id=…`。
//! 身份来自 `_ctx`（桥注入）；人面没有 external_url → 自然为 None。

/// Build a browser-viewable URL for a file under the sender data directory.
///
/// `rel_under_sender` is relative to `sender_data_dir` (e.g. `output/foo.md`).
/// Returns `None` if `external_url` is missing/empty or required identity
/// fields are empty.
...
mod tests {
    ...
    #[test]
    fn builds_plain_view_url_no_markdown() {
        let u = build_file_view_url(
    ...
    fn user_write_rel_paths() {
        ...
        assert_eq!(rel_path_for_user_write("/abs/path.md"), None);
    }
}

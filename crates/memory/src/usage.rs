...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> CarrierResult<()> {
        let conn = self
            .conn
            .lock()
        ...
        conn.execute(
            "INSERT INTO usage_events (id, agent_id, timestamp, model, input_tokens, output_tokens, tool_calls)
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> CarrierResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CarrierError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> CarrierResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CarrierError::Internal(e.to_string()))?;

        let sql = "SELECT model, COALESCE(SUM(input_tokens), 0),
        ...
        let row_data: Vec<rusqlite::Result<ModelUsage>> = stmt
            .query_map([], |row| {
                Ok(ModelUsage {
                    model: row.get(0)?,
    ...
    pub fn query_daily_breakdown(&self, days: u32) -> CarrierResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| CarrierError::Internal(e.to_string()))?;

        // SECURITY: `days` is u32, safe to format directly as it produces only digits.
        let sql = format!(
